...
/* =========================================================
   FIX: mat2 array removed (Veil GLSL crash)
   ========================================================= */
...
void main(void)
{
    ...
    /* darker base tone (kept your style) */
    vec4 col = vec4(0.2, 0.0, 0.0, 1.0);

    float pulse = fract(time / 400.0);

    col.g = sin(pulse * M_PI * 2.0) * 0.025 + 0.075;
    col.b = cos(pulse * M_PI * 2.0) * 0.02 + 0.05;
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        /* =========================================================
           FIX: remove pow() chain → stable hash
           ========================================================= */

        int j = i + 7;

        float base = float(j * 97 + i * 31);
        ...
        if (symbol >= 0 && symbol < cosmiccount)
        {
            ...
            float r = fract(rand1 * 0.00001) * 0.4 + 0.6;
            float g = fract(rand2 * 0.00001) * 0.3;
            float b = fract(rand1 * 0.00002) * 0.2;

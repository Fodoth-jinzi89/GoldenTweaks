...
/* =========================================================
   FIX: glsl.processor bug → remove mat2 array
   ========================================================= */
...
/* =========================================================
   OPT: faster rotation (reuse axis normalization result)
   ========================================================= */
...
void main(void)
{
    ...
    /* yaw/pitch rotation (unchanged but stable) */
    float sb = sin(pitch);
    float cb = cos(pitch);
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        /* OPT: remove heavy pow() chain */
        int j = i + 7;

        float rand1 = float(j * j * 4321 + j * 8);
        float rand2 = float(j * j * j * 239 + j * 37);
        float rand3 = rand1 * 0.0001 + rand2 * 0.00001;
        ...
        if (symbol >= 0 && symbol < cosmiccount)
        {
            ...
            float b = fract(rand1 * 0.00002) * 0.3 + 0.7;
